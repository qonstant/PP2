...
class MegaCoin(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pg.transform.scale(pg.image.load("./img/dcoin.png"), (60, 60))
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(60, WIDTH - 60), -300)
...
def main():
    ...
    while running:
        ...
        for enemy in enemies:
            enemy.update()
        for coin in megacoins:
            coin.update()
        C1.update()
        if LEVEL < SCORE // 5:
            LEVEL = SCORE // 5
            if random.randint(0, 5) in [i for i in range(0, 3)]:
                M1 = MegaCoin()
                megacoins.add(M1)
            STEP += 0.5
        # Collision ----------------------------------------------------------------------------------------------------
        ...
        if pg.sprite.spritecollideany(P1, coins):
            ...
            C1 = Coin()
            coins.add(C1)
        if pg.sprite.spritecollideany(P1, megacoins):
            SCORE += 10
            pg.mixer.Sound("./sound/coin.wav").play()
            # Deleting and adding New Megacoin -------------------------------------------------------------------------
            for c in megacoins:
                c.kill()
        SURF.blit(bg, (0, 0))
        # Drawing ------------------------------------------------------------------------------------------------------
